...
class FlowMatchActionHead(nn.Module):
    ...
    def loss(
        ...
        # --- COT: observation clustering for conditional OT ---
        token_group = transformer_outputs[self.readout_key]
        embeddings = token_group.tokens.mean(axis=-2)
        obs_flat = embeddings.mean(axis=1)  # (batch, emb_dim)

        # PCA + K-means (batch-size safe)
        n_comp = min(32, batch_size)
        n_clust = min(64, batch_size)
        mean, components = fit_pca(obs_flat, n_components=n_comp)
        obs_reduced = transform_pca(obs_flat, mean, components)
        centroids = fit_kmeans(obs_reduced, n_clusters=n_clust)
        ...
        # --- Optimal Transport coupling via Hungarian algorithm ---
        ...
        # Use pure_callback so the Hungarian algorithm works under JIT/grad
        def _solve_ot(cost_matrix):
            _, col_ind = linear_sum_assignment(np.asarray(cost_matrix))
            return np.array(col_ind, dtype=np.int32)

        pi = jax.pure_callback(
            _solve_ot,
...
class SimpleFlowMatchActionHead(nn.Module):
    """Predicts actions using vanilla flow matching (NO optimal transport).

    Same architecture and inference as FlowMatchActionHead, but training uses
    random noise pairing instead of COT coupling. This isolates the effect of
    the OT coupling on action prediction quality.
    """
    ...
    def loss(
        ...
        # Simple flow matching: random noise pairing (no OT)
        noisy_actions = (1 - time) * noise + time * actions_flat[None]
        v_target = actions_flat[None] - noise
...
class FPOActionHead(nn.Module):
    """Flow matching action head trained with Flow Policy Optimization (FPO).

    Architecture, BC `loss`, and `predict_action` are IDENTICAL to
    SimpleFlowMatchActionHead — FPO changes ONLY the RL fine-tuning
    objective, so inference cost is unchanged (this is the core claim of
    arXiv:2507.21053). The FPO objective itself lives on this head:

      - `cfm_loss_per_sample`: per-sample flow-matching loss for one
        (time, noise) draw — the quantity whose change defines the ratio.
      - `fpo_ratio_from_diff`: r̂ = exp(L_old - L_new), the tractable
        surrogate for the policy likelihood ratio.
      - `ppo_clip_loss`: the PPO-clip surrogate using that ratio.

    The old-vs-new parameter orchestration stays in the training loop
    (a single bound module cannot hold two parameter sets).
    """
    ...
    # FPO hyperparameters — self-documenting on the model component.
    fpo_clip_eps: float = 0.05
    fpo_n_mc: int = 4
    ...
    # ── FPO objective (RL fine-tuning only; not used at inference) ──

    def cfm_loss_per_sample(
        self,
        transformer_outputs: Dict[str, TokenGroup],
        act_batch: ArrayLike,
        taus: ArrayLike,
...
class SinkhornFlowMatchActionHead(nn.Module):
    """Flow matching with DIFFERENTIABLE Sinkhorn OT coupling.

    Key difference from FlowMatchActionHead:
      - Uses Sinkhorn iterations instead of Hungarian algorithm
      - Fully differentiable — gradients flow through the OT coupling
      - Pure JAX — no scipy callbacks, JIT-compiles cleanly
      - The model learns to produce embeddings that improve the coupling

    The Sinkhorn coupling produces a soft permutation matrix that matches
    noise samples to action targets, creating straighter flow paths.
    """
    ...
    def loss(
        ...
        # --- Differentiable Sinkhorn OT coupling ---
        # Match noise to actions using Sinkhorn (fully differentiable)
        noise_2d = noise[0].reshape(batch_size, -1)
        actions_2d = actions_flat.reshape(batch_size, -1)
        matched_noise_2d = sinkhorn_match_noise(
            noise_2d, actions_2d,
            reg=self.sinkhorn_reg,
            n_iters=self.sinkhorn_iters,
        )
        # Reshape back and replace noise[0]
        matched_noise = matched_noise_2d.reshape(noise[0].shape)
        noise = noise.at[0].set(matched_noise)
...
class ConditionedOTFlowMatchActionHead(nn.Module):
    """Flow matching with History- and Context-Conditioned Optimal Transport.

    Novel contribution (Sarvik'24): The OT coupling cost incorporates not just
    action-space distance, but also observation context similarity and optional
    temporal history. This prevents cross-context noise-action matching:

        cost(i,j) = ||noise_i - action_j||²                    (geometric)
                   + λ_obs * ||obs_embed_i - obs_embed_j||²    (context)
                   + λ_hist * ||history_i - history_j||²        (temporal)

    Why this matters for robotics:
      - In multi-task learning, prevents reach-noise → push-action matches
      - Preserves temporal dynamics (nearby timesteps matched together)
      - Pure JAX / JIT-compilable via Sinkhorn (no scipy callbacks)

    The observation conditioning creates task-local OT: within each context
    cluster, the coupling straightens flow paths. Across contexts, the penalty
    prevents unrealistic couplings that would confuse the velocity field.
    """
    ...
    use_layer_norm: bool = True
    flow_steps: int = 10
    n_flow_samples: int = 2  # 1 OT-matched + 1 random for robustness

    # Conditioned OT hyperparameters
    sinkhorn_reg: float = 0.05
    sinkhorn_iters: int = 20
    obs_weight: float = 1.0       # observation context penalty weight
    history_weight: float = 0.5   # temporal history penalty weight
    use_history: bool = True      # whether to use temporal features
    ...
    def _build_history_features(self, actions_flat, window_size):
        """Build temporal history features from action sequence.

        Uses the action trajectory across the window as a temporal signature.
        For window=1, falls back to the action itself.
        """
        if window_size <= 1:
            return None
        # Use the difference between consecutive window steps as velocity signal
        # actions_flat: (batch, window, action_dim*horizon)
        deltas = actions_flat[:, 1:, :] - actions_flat[:, :-1, :]  # (B, W-1, D)
        # Mean temporal velocity as a compact history feature
        history = deltas.mean(axis=1)  # (B, D)
        return history
    ...
    def loss(
        ...
        # Get observation embeddings for conditioned OT
        token_group = transformer_outputs[self.readout_key]
        if self.use_map:
            obs_embeds = self.map_head(token_group, train=train)[:, :, 0]
        else:
            obs_embeds = token_group.tokens.mean(axis=-2)  # (B, W, emb_dim)
        # Flatten obs embeddings across window for per-sample context vector
        obs_embeds_flat = obs_embeds.reshape(batch_size, -1)  # (B, W*emb_dim)

        actions_flat = rearrange(actions, "b w h a -> b w (h a)")
        actions_flat = jnp.clip(actions_flat, -self.max_action, self.max_action)

        # Build temporal history features
        history_embeds = None
        if self.use_history:
            history_embeds = self._build_history_features(actions_flat, window_size)
        ...
        # --- History-Conditioned Sinkhorn OT ---
        noise_2d = noise[0].reshape(batch_size, -1)
        actions_2d = actions_flat.reshape(batch_size, -1)

        matched_noise_2d = conditioned_sinkhorn_match(
            noise_2d,
            actions_2d,
            obs_embeddings=obs_embeds_flat,
