...
class DiffusionActionHead(nn.Module):
    ...
    def loss(
        ...
        noise = jax.random.normal(
            noise_key, (self.n_flow_samples,) + actions_flat.shape
        )
        ...
        # add a dimension to the mask for n_flow_samples
        mask = mask[None]
...
class FlowMatchActionHead(nn.Module):
    ...
    def setup(self):
        # Create the flow model (Score network)
    ...
    def __call__(
        ...
        if (time is None or noisy_actions is None) and not self.is_initializing():
            raise ValueError(
                "Must provide time and noisy_actions when calling flow action head"
            )
        elif self.is_initializing():
            time = jnp.zeros((*embeddings.shape[:2], 1), dtype=jnp.float32)
            noisy_actions = jnp.zeros(
        ...
        pred_vel = self.flow_model(embeddings, noisy_actions, time, train=train)
        return pred_vel
    ...
    def loss(
        ...
        # piggy-back on the dropout rng chain for flow rng
        rng = self.make_rng("dropout")
        time_key, noise_key = jax.random.split(rng)
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        def scan_fn(carry, time):
            current_x, rng = carry
            input_time = jnp.broadcast_to(jnp.array(time, dtype=jnp.float32),(*current_x.shape[:-1], 1))
            ...
            # set non-eval actions to the noise that would have been seen during training
            current_x = jnp.where(
                flat_action_mask, current_x, noise
            )
    ...
    def predict_action(
        ...
        **kwargs,
        ...
        def loop_body(i, args):
            ...
            orig = (sample - jnp.sqrt(1 - alpha_prod_t) * vel) / jnp.sqrt(alpha_prod_t)
